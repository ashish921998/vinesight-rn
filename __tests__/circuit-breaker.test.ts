...
describe('CircuitBreaker', () => {
  ...
  it('CircuitBreakerOpenError contains breaker name and remaining time', async () => {
    const cb = new CircuitBreaker({ name: 'my-breaker', failureThreshold: 1, cooldownMs: 60_000 });
    ...
    try {
      ...
      expect(cbError.breakerName).toBe('my-breaker');
      expect(cbError.remainingMs).toBeGreaterThan(0);
      expect(cbError.remainingMs).toBeLessThanOrEqual(60_000);
    }
  });
});
