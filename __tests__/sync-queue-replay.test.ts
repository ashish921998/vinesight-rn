 ...
 * failure tracking, reset behaviour, and edge cases.
 *
...
describe('Sync Queue', () => {
  ...
  it('does not crash when marking a non-existent item as failed', () => {
    useSyncStore.getState().markFailed('non-existent', 'Error');
    const snapshot = getSyncQueueSnapshot();
    expect(snapshot.totalCount).toBe(0);
  });

  it('does not crash when marking a non-existent item as synced', () => {
    useSyncStore.getState().markSynced('non-existent');
    const snapshot = getSyncQueueSnapshot();
    expect(snapshot.totalCount).toBe(0);
  });

  it('does not crash when removing a non-existent item', () => {
    useSyncStore.getState().removeItem('non-existent');
    const snapshot = getSyncQueueSnapshot();
    expect(snapshot.totalCount).toBe(0);
  });

  it('handles large queue sizes', () => {
  ...
  it('correctly counts mixed statuses', () => {
    ...
    expect(snapshot.pendingCount).toBe(2);
    expect(snapshot.failedCount).toBe(1);
    expect(snapshot.syncingCount).toBe(1);
