/**
 * Router Module
 * Determines the route for chat messages: advisory, voice_log, farm_query, clarify_route, fallback_llm.
 * This is the clean, modular replacement for the monolithic voice-routing.ts.
 */
...
function parseHarvestGrade(transcript: string): string | null {
  ...
  for (const grade of HARVEST_GRADES) {
    ...
    if (normalizedGrade.length > 1 && normalized.includes(normalizedGrade)) {
      return grade;
    }
  }
...
export function decideChatRoute(input: {
  ...
  // If there's an active draft, prioritize voice_log unless there's strong alternate intent
  if (hasActiveDraft) {
...
// ============================================================
// MARK: - Voice Log Utilities
// ============================================================

/**
 * Find farm matches by name
 */
...
export function resolveVoiceLogTurn(input: {
  ...
  if (llmExtraction?.cancel || VOICE_PATTERNS.cancel.some((pattern) => pattern.test(text))) {
  ...
  // If no draft and likely a history query, skip
  ...
  // Merge extraction and text data
  ...
  if (parsedDate) {
    nextDraft = { ...nextDraft, date: parsedDate };
  }

  const missingFields = getVoiceLogMissingFields(nextDraft);
  if (missingFields.length > 0) {
    return { kind: 'clarify', draft: nextDraft, missingFields };
  }
...
export function shouldAttemptVoiceLogExtraction(
  ...
  // Skip for very short inputs without keywords
...
/**
 * Build voice log clarification message
 */
...
/**
 * Build clarify exhausted message
 */
...
/**
 * Build route clarification prompt
 */
...
/**
 * Build route clarification retry message
 */
...
/**
 * Build route clarification cancelled message
 */
