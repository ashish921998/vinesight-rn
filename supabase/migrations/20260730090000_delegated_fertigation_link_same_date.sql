-- Tighten the delegated fertigation link validation: the referenced irrigation
-- record must match p_date as well as p_farm_id. Every legitimate flow creates
-- the pair in one same-date save, so a cross-date link is always bogus input —
-- and the UI (timeline fold, edit-form date sync) assumes linked pairs share a
-- date. Same foreign-key-style error as the farm check.
  ...
  case p_record_type
    ...
    when 'fertigation' then
      ...
      if linked_irrigation_id is not null and not exists (
        ...
        where ir.id = linked_irrigation_id and ir.farm_id = p_farm_id and ir.date = p_date
      ) then
        raise exception 'Linked irrigation record does not belong to this farm and date' using errcode = '23503';
      end if;
