...
begin
  ...
  on conflict (user_id) do update set
    ...
    updated_at = now()
  -- Design note: The case-when pattern for nullable fields prevents explicit NULL clearing - it always
  -- preserves existing values when the parameter is NULL (whether not provided or explicitly passed).
  -- This is intentional for guided tour: timestamps and tracking fields should only be set, never cleared.
