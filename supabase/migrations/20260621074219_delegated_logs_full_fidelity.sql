-- Bring delegated (professional-created) logs to full parity with farmer-created logs.
-- The original create_delegated_log (202606210001) stored only a flattened `chemical`
-- string for spray and ignored chemical_items / nutrient totals / date_of_pruning.
-- Now that the professional add-log screen reuses the same farmer form components, those
-- forms produce the rich payload, so the RPC persists it identically. date_of_pruning is
-- sourced server-side from the farm row (the farmer path's source of truth), so the client
  ...
  case p_record_type
    ...
    when 'fertigation' then
      ...
      insert into public.fertigation_records
        (farm_id, season_id, date, fertilizers, water_volume, nutrient_totals_elemental, nutrient_totals_elemental_per_acre, nutrient_calc_coverage, area, date_of_pruning, notes, professional_creator_id, acting_organization_id, professional_creator_name, acting_organization_name)
      values (p_farm_id, resolved_season_id, p_date, coalesce(p_payload->'fertilizers','[]'::jsonb), nullif(p_payload->>'water_volume','')::numeric,
        p_payload->'nutrient_totals_elemental', p_payload->'nutrient_totals_elemental_per_acre', nullif(p_payload->>'nutrient_calc_coverage','')::numeric,
        (p_payload->>'area')::numeric, farm_date_of_pruning, p_payload->>'notes', auth.uid(), p_organization_id, creator_name, organization_name)
