-- Incremental delta for environments where prior migrations are already applied.
-- Includes:
-- 1) guided-tour RPC update: last_active_at should not be cleared by p_clear_nullable_fields
-- 2) push-devices uniqueness update: unique by (user_id, expo_push_token)

begin;

-- ---------------------------------------------------------------------------
-- guided_tour RPC delta
-- ---------------------------------------------------------------------------
...
begin
  ...
  on conflict (user_id) do update set
    ...
    last_active_at = coalesce(p_last_active_at, public.user_guided_tour_state.last_active_at),
...
-- ---------------------------------------------------------------------------
-- user_push_devices unique-constraint delta
-- ---------------------------------------------------------------------------
