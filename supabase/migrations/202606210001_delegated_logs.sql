...
begin
  foreach table_name in array array[
    ...
    execute format(
      'alter table public.%I add column if not exists professional_creator_id uuid references auth.users(id) on delete restrict, add column if not exists acting_organization_id uuid references public.organizations(id) on delete restrict',
      table_name
    );
    execute format(
  ...
  case p_record_type
    when 'irrigation' then
      ...
      insert into public.irrigation_records
        (farm_id, date, duration, area, growth_stage, moisture_status, system_discharge, notes, professional_creator_id, acting_organization_id)
      select p_farm_id, p_date, (p_payload->>'duration')::numeric, f.area,
        coalesce(p_payload->>'growth_stage', ''), coalesce(p_payload->>'moisture_status', ''),
        coalesce(f.system_discharge, 0), p_payload->>'notes', auth.uid(), p_organization_id
    ...
    when 'spray' then
      ...
      insert into public.spray_records
        (farm_id, date, chemical, dose, area, weather, operator, notes, professional_creator_id, acting_organization_id)
      values (p_farm_id, p_date, trim(p_payload->>'chemical'), coalesce(p_payload->>'dose',''), (p_payload->>'area')::numeric,
        coalesce(p_payload->>'weather',''), coalesce(p_payload->>'operator',''), p_payload->>'notes', auth.uid(), p_organization_id)
    ...
    when 'fertigation' then
      ...
      insert into public.fertigation_records
        (farm_id, date, fertilizers, water_volume, area, notes, professional_creator_id, acting_organization_id)
      values (p_farm_id, p_date, coalesce(p_payload->'fertilizers','[]'::jsonb), nullif(p_payload->>'water_volume','')::numeric,
        (p_payload->>'area')::numeric, p_payload->>'notes', auth.uid(), p_organization_id)
    ...
    when 'harvest' then
      ...
      insert into public.harvest_records
        (farm_id, date, quantity, grade, price, buyer, notes, professional_creator_id, acting_organization_id)
      values (p_farm_id, p_date, (p_payload->>'quantity')::numeric, coalesce(p_payload->>'grade',''), nullif(p_payload->>'price','')::numeric,
        p_payload->>'buyer', p_payload->>'notes', auth.uid(), p_organization_id)
    ...
    when 'note' then
      ...
      insert into public.daily_notes
        (farm_id, date, notes, professional_creator_id, acting_organization_id)
      values (p_farm_id, p_date, trim(p_payload->>'notes'), auth.uid(), p_organization_id)
