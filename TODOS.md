...
**Context:** The delegated path writes through RPCs (create_delegated_log / update_delegated_log / delete_delegated_log, src/services/delegated-logs.ts) but lands in the SAME six tables, which already carry professional_creator_id / acting_organization_id / *_name attribution columns. So it can reuse the same identity + paused-mutation queue model; the offline mutation just needs to call the delegated RPC (with attribution) instead of the direct insert. client_uuid applies only to the five surrogate-keyed event tables — daily_notes stays idempotent via its natural key UNIQUE(farm_id, date), no client_uuid needed.

**Depends on / blocked by:** v1 offline (farmer path) landing first. Requires the five event-table delegated RPCs to accept client_uuid and upsert on it (daily_notes RPC upserts on (farm_id, date) as it already does).
