...
export default function WarehouseItemForm({
  ...
  const selectedCatalogueLabel = useMemo(() => {
    ...
    return preset ? `${preset.label} – ${preset.manufacturer}` : '';
  }, [selectedCatalogueId]);
  ...
  const handleSelectCatalogueItem = (preset: WarehouseNutrientPreset) => {
    applyPreset(preset);
  ...
  return (
    ...
    >
      ...
      {/* Catalogue trigger button – replaces native Picker for cross-platform consistency */}
      <Pressable
        style={{
      ...
      >
        <Text
          style={{
            ...
            color: selectedCatalogueLabel
              ? colors.surface[900]
              : colors.surface[400],
            fontWeight: selectedCatalogueLabel
              ? fontWeight.medium
              : fontWeight.normal,
            flex: 1,
          }}
          numberOfLines={1}
        >
          {selectedCatalogueLabel || 'Select from catalogue (or skip)'}
        </Text>
    ...
    {/* Catalogue picker bottom-sheet overlay */}
    <Modal
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <IconSymbol name="xmark" size={20} color={m3.colorScheme.onSurface} />
            </Pressable>
          </View>
          ...
          <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled">
            <Pressable
              style={{
                ...
                backgroundColor:
                  !selectedCatalogueId ? colors.surface[50] : colors.surface[100],
              }}
              onPress={handleClearCatalogue}
            >
              <View
                style={{
                  flexDirection: 'row',
              ...
              >
                ...
                {!selectedCatalogueId && (
                  <IconSymbol name="checkmark" size={20} color={colors.primary[500]} />
                )}
              </View>
            </Pressable>
            ...
            {visibleCatalogueItems.map((preset) => {
              const isSelected = selectedCatalogueId === preset.id;
              return (
                <Pressable
                  key={preset.id}
                  style={{
                  ...
                  onPress={() => handleSelectCatalogueItem(preset)}
                >
                  <View
                    style={{
                      flexDirection: 'row',
