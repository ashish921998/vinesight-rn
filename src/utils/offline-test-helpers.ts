...
export function seedSyncQueue(
  ...
  for (const item of items) {
    store.upsertItem(item.id, {
      ...
      ...(item.queuedAt ? { queuedAt: item.queuedAt } : {}),
    });
  }
}
...
export function getSyncQueueSnapshot(): {
  ...
  return {
    items,
    pendingCount: values.filter((i) => i.status === 'pending').length,
    failedCount: values.filter((i) => i.status === 'failed').length,
    syncingCount: values.filter((i) => i.status === 'syncing').length,
 ...
 */
export function createFailThenSucceedFn<T>(
  failCount: number,
  successValue: T,
