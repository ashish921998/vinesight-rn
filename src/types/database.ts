...
/**
 * Format a Date as a Supabase date string (YYYY-MM-DD) using the LOCAL
 * calendar day — not UTC. The previous `.toISOString()` implementation shifted
 * the day backward in positive-offset timezones (e.g. local Jul 8 00:00 in
 * UTC+5:30 serialized to "2026-07-07"), which mis-keyed the week-strip dots and
 * could persist logs with the wrong date. Local-component formatting matches the
 * existing `formatLocalDate` helper and the dates the DB already stores.
 */
